...
public class PictureTest {
	...
	private List<Sphere> initializeBalls() {
		...
		for (double i = 0, j = 6; i < 500; i += 1, j += 0.04) {
			double x = Math.cos(i);
			double y = Math.sin(i);
			balls.add((Sphere) new Sphere(j, new Point(i * y, i * x, (i * 2 - 150))).setMaterial(material1)
	...
	public void pictureTest() {
		...
		camera.setImageWriter(new ImageWriter("final picture", 500, 500)).setRayTracer(new RayTracerBasic(scene))
